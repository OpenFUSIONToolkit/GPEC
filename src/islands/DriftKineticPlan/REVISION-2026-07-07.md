...
The original ISLET design bundle (drafted in Claude chat from memory of the
literature) was revised against a full read of the nine PDFs in
`docs/resources/Drift_Kinetic_Island_References/` plus a reality check of the
GPEC repo's actual module status. The live copy of the plan is the unpacked
tree `islet-design-docs/islet/` (the only copy — the loose top-level
duplicates of CLAUDE.md/00/07 that came with the original chat export were
removed); `islet-design-docs.zip` is the untouched pre-revision original for
reference.
...
drift-kinetic NTM threshold code — effectively a working prototype of ISLET's
Level 0 — with:
...
- forensic documentation of the numerical failure modes: intrinsically
  **singular trapped-passing matching matrix** (TSVD required; new ladder A8
  ...
  each maps onto an ISLET architecture choice (matrix-free Newton–Krylov,
...
physics question ISLET can settle (E4/E6; Paper I candidate result).
...
**Sequencing decision:** #238 lands before ISLET M0; if ISLET starts earlier,
the ISLET branch is cut from `feature/tearing-growthrates` rather than
...
New `[CHECKED: source, Eq./p.]` state in the islet CLAUDE.md: transcribed from
