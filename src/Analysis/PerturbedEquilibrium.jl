"""
    PerturbedEquilibriumAnalysis

Post-processing utilities for converting modal GPEC outputs to theta-space
and (theta, phi) grids.
"""
module PerturbedEquilibriumAnalysis

using HDF5
using FastInterpolations: cubic_interp, CubicFit, PeriodicBC, ExtendExtrap, WrapExtrap,
                          LinearBinary
...
function modes_to_theta(h5_file::String, variable::String;
    ...
    h5open(h5_file, "r") do f
        ...
        if !keep_sfl_phi
            # Reconstruct ν spline from stored grid + nodal values
...
end # module PerturbedEquilibriumAnalysis
