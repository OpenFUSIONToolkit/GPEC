...
function fourier_decompose_bn(
    ...
    exp_basis = compute_fourier_coefficients(mtheta, mpert, m_low, nzeta, 1, n)

    bn_flat = vec(bn)  # column-major: bn_flat[i + (j-1)*mtheta] = bn[i,j] ✓
    scale = 2.0 / (mtheta * nzeta)

    bmn_real = scale .* (real(exp_basis)' * bn_flat)
    bmn_imag = scale .* (-imag(exp_basis)' * bn_flat)
...
function convert_forcing_normalization!(
    ...
    bn_hat = (real(exp_basis) * amp_real .- imag(exp_basis) * amp_imag)  # length mtheta*nzeta
    ...
    scale = 2.0 / (mtheta * nzeta)
    new_real = scale .* (real(exp_basis)' * bn_flat)
    new_imag = scale .* (-imag(exp_basis)' * bn_flat)
