...
function initialize_wall(inputs::VacuumInput, plasma_surf::PlasmaGeometry, wall_settings::WallShapeSettings)
   ...
   # All of these arrays are of length mtheta with θ = [0, 1)
    ...
    if any(x_wall .<= 0.0) && !nowall
        # to add support for x<0 walls, be sure to carefully replicate [Chance Phys. Plasmas 1997 2161] fortran code x<0 handling in the kernel function to account for the additional singularities associated with this
