...
| `omega`   | E×B rotation Ω_E, per unit n            | rad/s |

Construct via the keyword constructor `KineticProfiles(; psi, n_e, T_e, T_i, omega)` with
...
```julia
vals = kp(0.5)    # NamedTuple(n_e=..., T_e=..., T_i=..., omega=...)
```
"""
