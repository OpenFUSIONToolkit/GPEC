...
function main(args::Vector{String}=String[]; dd::Union{IMASdd.dd,Nothing}=nothing)
    ...
    # Energy-principle reconstruction (DCON recon flag): real-space v1 (gpout_recon)
    # + matrix-form v2 for the least-stable eigenmode. Written to a standalone
    # recon.h5 file, and mirrored into the gpec.h5 recon/ group for regression.
    if ctrl.recon_flag && ctrl.vac_flag && vac_data !== nothing
        @info "Reconstructing energy principle (recon_flag)"
        recon_start = time()
