...
function main(args::Vector{String}=String[])
    ...
    equil_force_termination = get(get(inputs, "Equilibrium", Dict()), "force_termination", false)
    if ctrl.use_equal_arc_vacuum && !ctrl.force_termination && !equil_force_termination && "PerturbedEquilibrium" in keys(inputs)
