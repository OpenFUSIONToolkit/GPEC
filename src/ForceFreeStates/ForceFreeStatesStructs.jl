...
@kwdef mutable struct ForceFreeStatesControl
    ...
    bvp_riccati::Bool = false      # EXPERIMENTAL within use_bvp: collocate the Riccati matrix W=U₁U₂⁻¹ (W(axis)=0) instead of the raw fundamental matrix. W is bounded for STABLE segments → well-conditioned, but has a pole at any conjugate point (i.e. exactly where the mode is unstable, U₂ singular) and the N²-per-node collocation Jacobian is dense (OOMs at production node counts; needs the analytic Kronecker jac). Default off; see PR #256 for the conjugate-point obstacle.
