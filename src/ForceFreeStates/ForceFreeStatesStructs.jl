...
## Fields
  ...
  - `parallel_threads::Int` - Toggle for the parallel-FM chunk integration. **Default `2`.** Any value `> 1` enables the parallel path: the chunk loop runs with `Threads.@threads :dynamic` across **all** `Threads.nthreads()` workers (this field does *not* cap them — to cap, start Julia with `julia -t N`), and the standard-EL `u_store` pass runs concurrently via `Threads.@spawn`. Set `parallel_threads = 1` to run the FM chunks SERIALLY after the standard pass — this is bit-deterministic and immune to the thread-schedule sensitivity that historically caused intermittent BVP divergences on numerically delicate equilibria like DIII-D 147131 (see CONVENTIONS.md §7). Empirical reliability sweep (5 trials × {1,2,4} on DIII-D 147131 βₚ≈0.07): 15/15 bit-identical Δ′ at every setting. If a parallel run diverges, drop to `parallel_threads = 1` rather than switching `use_parallel = false` — the latter is silently wrong.
"""
