...
function prepare_ballooning_coefficients(ipsi::Int, plasma_eq::Equilibrium.PlasmaEquilibrium, hint::Ref{Int})
    ...
    spl0_interp = cubic_interp(theta_grid, hcat(1 ./ bsq, jac .* b1 ./ bsq); search=LinearBinarySearch(), bc=PeriodicBC())
    ...
    spl1_interp = cubic_interp(theta_grid, spl1_fs; search=LinearBinarySearch(), bc=PeriodicBC())
