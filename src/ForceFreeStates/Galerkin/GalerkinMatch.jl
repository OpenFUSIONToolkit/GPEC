...
Requires `gal_result.solution` (reconstructed outer ξ/ξ′), the rpec coil block `gal_result.delta_coil`,
and per-surface resistive inputs `ctrl.gal_eta`/`gal_rho`/`gal_rotation` (length `msing`) + `ctrl.gal_gamma`.
"""
function gal_match_rpec(ctrl::ForceFreeStatesControl, equil, intr::ForceFreeStatesInternal,
    ...
    # --- matched outer ξ/ξ′ per coil drive (match.f:1654-1671, resistive branch) ---
