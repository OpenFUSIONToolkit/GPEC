...
## Fields
  ...
  - `psilim_from_layer_overlap::Bool` - Cap the integration domain at the point where adjacent rational surfaces' resistive layers overlap, after Fitzpatrick, Nucl. Fusion 2025 Sect. 5.9. Past that point no surface retains a well-separated inner region, so the matched-asymptotic treatment is not defined there. Applied as an upper bound on `qlim` at the top of `sing_lim!`, so `dmlim` / `qhigh` still select the final surface from inside it, and a bound beyond `psihigh` is inert. On the shipped DIII-D-like decks the overlap point sits near `psi = 0.9997`, beyond every deck's `psihigh`, so the cap is inert there and enabling the flag changes nothing; it binds only once `psihigh` is far enough out that `psilim` would otherwise land past the overlap point. The scan needs kinetic profiles and is run whenever they are readable — its result is recorded under `ForceFreeStates/LayerOverlap/` regardless of this flag, so a run always shows whether layer physics would have constrained the domain. For multi-`n` runs the scan is repeated for each `n` in `nn_low:nn_high` and the innermost overlap point binds. Default `false`.
