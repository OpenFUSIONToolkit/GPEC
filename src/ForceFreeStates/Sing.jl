...
# Geometric knot ladder straddling each rational surface (see `rational_psi_ladder`). The
# innermost spacing `SING_LADDER_H0` is a fixed absolute ψ_N step — independent of the global
# accuracy target τ, so the singular-layer stencil (and hence Δ′) is τ-invariant — chosen to
# match the equilibrium's native radial resolution. It must NOT be pushed below that
# resolution (e.g. down to the Δ′ matching offset singfac_min/|n·q1| ~ 1e-5): the equilibrium
# is smooth through a rational, so sub-resolution knots merely make the re-splined equilibrium
# ring, and the Δ′ small/large-solution cancellation amplifies that noise into a spurious Δ′.
const SING_LADDER_H0 = 1.0e-3    # innermost ladder spacing (absolute ψ_N step at the surface)
const SING_LADDER_GROWTH = 2.0   # geometric growth of ladder spacing away from the surface
const SING_LADDER_KMAX = 12      # cap on ladder knots per side
const SING_LADDER_RADIUS = 0.05  # ψ_N half-width of the packed region around each rational

"""
    rational_psi_ladder(equil::Equilibrium.PlasmaEquilibrium; nlow::Int, nhigh::Int=nlow,
                        singfac_min::Float64, h0=SING_LADDER_H0, growth=SING_LADDER_GROWTH,
                        kmax=SING_LADDER_KMAX, radius=SING_LADDER_RADIUS)

Mandatory ψ_N knots for the two-pass grid refinement: each rational surface q = m/n gets a
symmetric geometric ladder straddling it, so the Δ′ singular-layer stencil is resolved
identically regardless of the global accuracy target τ (`psi_accuracy`). Knots sit at ψ_r,
ψ_r ± `h0`, ψ_r ± `h0`·`growth`, … out to `radius`, with a fixed absolute innermost spacing
`h0` chosen to match the equilibrium's native radial resolution. A τ- (and grid-topology-)
invariant stencil is what makes Δ′ grid-robust; `h0` must stay at the equilibrium resolution,
not the far finer perturbed-ODE matching offset (see the note on `SING_LADDER_H0`). Returns a
sorted, unique vector including the exact rationals; falls back to the bare rationals when
`singfac_min <= 0` (no surface crossing, so no Δ′ layer to resolve).
"""
function rational_psi_ladder(equil::Equilibrium.PlasmaEquilibrium; nlow::Int, nhigh::Int=nlow,
    singfac_min::Float64, h0::Float64=SING_LADDER_H0, growth::Float64=SING_LADDER_GROWTH,
    kmax::Int=SING_LADDER_KMAX, radius::Float64=SING_LADDER_RADIUS)
    singfac_min > 0 || return rational_psi_nodes(equil; nlow=nlow, nhigh=nhigh)
    lo, hi = first(equil.profiles.xs), last(equil.profiles.xs)
    knots = Float64[]
    is_rational = Bool[]
    qs = Float64[]
    for s in _find_rational_surfaces(equil, nlow, nhigh)
        q = s.m / s.n
        any(qq -> isapprox(qq, q; atol=1e-8), qs) && continue
        push!(qs, q)
        push!(knots, s.psifac)
        push!(is_rational, true)
        d = h0
        for _ in 1:kmax
            d > radius && break
            push!(knots, s.psifac + d)
            push!(knots, s.psifac - d)
            push!(is_rational, false)
            push!(is_rational, false)
            d *= growth
        end
    end
    # Thin to a native-resolution minimum spacing h0: overlapping ladders from nearby rationals
    # (low shear / multi-n) must not create sub-h0 intervals that ring the re-splined equilibrium.
    # Sweep in ψ order keeping a knot only if it is ≥ h0 from the last kept one; an exact rational
    # displaces a too-close ladder knot (a pair of rationals closer than h0 cannot be resolved
    # separately at this spacing, so only the first is kept).
    kept = Float64[]
    kept_rational = Bool[]
    for i in sortperm(knots)
        x = knots[i]
        (lo < x < hi) || continue
        if isempty(kept) || x - kept[end] >= h0
            push!(kept, x)
            push!(kept_rational, is_rational[i])
        elseif is_rational[i] && !kept_rational[end]
            kept[end] = x
            kept_rational[end] = true
        end
    end
    return kept
end
