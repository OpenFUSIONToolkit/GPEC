...
@with_pool pool function equilibrium_solver(raw_profile::DirectRunInput, fieldline_int=direct_fieldline_int)
    ...
    # Enforce periodic endpoint after all newq0 modifications are complete.
    # Bitwise copy guarantees the PeriodicBC(:inclusive) tolerance check passes.
    rzphi_nodes[:, end, :] .= rzphi_nodes[:, 1, :]
    ...
    eqfun_fs_nodes[:, end, :] .= eqfun_fs_nodes[:, 1, :]
