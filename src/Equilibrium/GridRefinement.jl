...
whose pedestal gradients attract knots; `mandatory` lists rational-surface ψ values to bracket (`bracket_mandatory = false` skips the bracketing and keeps the fine knots inside instead); `pinned` lists ψ values inserted as plain knots without a cleared zone (kinetic-resonance surfaces);
...
function refined_psi_grid(equil::PlasmaEquilibrium;
    ...
    # `bracket_mandatory = false` keeps the locally-uniform fine patch around each rational
    # instead of clearing it: the cleared zone exists solely so the ideal Δ′ stencil sees a
    # consistent 3rd derivative across the surface, so it is only worth its cost when that Δ′ is
    # actually delivered (kinetic runs withhold it — see PerturbedEquilibriumControl
    # `unvalidated_kinetic_tearing`). The knots inside are what resolve the kinetic matrices'
    # near-rational structure.
    (isempty(mandatory) || !bracket_mandatory) && return grid
