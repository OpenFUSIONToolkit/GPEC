...
function load_fortran_cumulative(case_dir::String, method::String, ell::Int)
    ncfile = joinpath(case_dir, "pentrc_output_n1.nc")
    var_psi = "psi_$(method)"
    var_t = "T_$(method)"
    text = read(`ncdump -v $(var_psi),ell,$(var_t) $ncfile`, String)
    ...
    psi = parse_ncdump_vector(data, var_psi; stop_label="$(var_t) =")
...
function write_svg(outfile::String, method::String, ell::Int, psi::Vector{Float64}, fortran::Vector{ComplexF64}, julia::Vector{ComplexF64})
    ...
    open(outfile, "w") do io
        ...
        println(io, @sprintf("""<text x="80" y="40" font-size="24" font-family="Helvetica" fill="#111">%s cumulative profile comparison (ell = %d)</text>""", uppercase(method), ell))
...
function main(case_dir::String, method::String, ell::Int, nsample::Int, outfile::String, electron::Bool)
    psi_all, fortran_all = load_fortran_cumulative(case_dir, method, ell)
    ...
    for ψ in psi
        val = redirect_stdout(devnull) do
            redirect_stderr(devnull) do
                P.tintgrl_lsode([0.0, ψ], 1, ell, 1, 2, 1.0, 1.0, electron, method)
            end
        end
        push!(julia, val)
    end
...
method = get(ENV, "PENTRC_METHOD", "fcgl")
ell = parse(Int, get(ENV, "PENTRC_ELL", method == "fcgl" ? "0" : "1"))
nsample = parse(Int, get(ENV, "PENTRC_POINTS", "20"))
outfile = get(ENV, "PENTRC_OUTFILE", joinpath(case_dir, "$(method)_cumulative_profile_compare.svg"))
