...
function compute_torque_all_methods!(intr::PentrcInternal, ctrl::PentrcControl)
    ...
    for m in 1:length(intr.methods)
        ...
        if method in ["clar", "rlar"]
            ...
            load_fnml!(joinpath(data_dir, "fkmnl.dat"))
        end
...
function compute_kinetic_contribution(ctrl::PentrcControl, equil, ffit)
    ...
    if hasproperty(ffit, :mpert)
        set_mode_numbers!(default_mode_numbers(ffit.mpert))
    end

    mpert = hasproperty(ffit, :mpert) ? ffit.mpert : 0
    zero_mat = zeros(ComplexF64, mpert, mpert)

    # Create result dictionary
    kinetic_results = Dict(
        :fmat_kin => copy(zero_mat),
        :gmat_kin => copy(zero_mat),
        :kmat_kin => copy(zero_mat)
    )
