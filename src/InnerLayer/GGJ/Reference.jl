...
D-shaped aspect-ratio-2, q = 2 surface from GW2020 Eq. 55 (companion operating
point `Q = 1.234e-1`, their Fig. 1); benchmark for the inps basis convergence
(their Figs. 1–4). Eq. 55 does not tabulate `Δ(Q)` — its `Δ_±` (Eq. 54) is a
convergence norm — so quantitative Δ cross-checks need a Fortran rmatch/inps
run. Timescales default to canonical normalization; override for physical cases.
"""
