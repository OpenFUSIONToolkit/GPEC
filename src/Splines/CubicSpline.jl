...
function spline_eval!(spline::SplineType, x::Float64)
    f = zeros(Float64, spline.nqty)
    call_spline_c_eval(Float64, spline, x, f)
    return f
end
...
function spline_deriv1!(spline::SplineType, x::Float64)
    f = zeros(Float64, spline.nqty)
    f1 = zeros(Float64, spline.nqty)
    call_spline_c_eval(Float64, spline, x, f, f1)
    return f, f1
end
...
function spline_fit!(spline::SplineType, bctype::String="periodic")
    bctype_int = parse_bctype(bctype)
    spline.bctype = Int32(bctype_int)
