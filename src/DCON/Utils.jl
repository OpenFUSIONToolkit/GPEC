...
function resize_storage!(odet::OdeState)
    oldlen = size(odet.u_store, 3)
    newlen = 2 * oldlen

    # Allocate new arrays
    u_new = Array{ComplexF64,3}(undef, 2*odet.numpert_total, odet.numpert_total, newlen)
    ud_new = Array{ComplexF64,3}(undef, 2*odet.numpert_total, odet.numpert_total, newlen)
