...
# count for the two integration paths: the chunked-Riccati default and the legacy
...
# to measure `forcefreestates_integration` — the ForceFreeStates stage whose cost the
# chunked-Riccati path parallelises — for the DIII-D-like example at n=1 and n=4
# (n=4 has more rational surfaces to cross).
#
# The legacy Euler-Lagrange path contains no `Threads.@threads`, so it is
# thread-independent: it is measured once (in the 1-thread worker) and drawn as a
# horizontal reference band. Only the chunked-Riccati path is swept across 1..N
# threads.
#
# Usage:
    ...
    for n in N_VALUES
        ...
        for mode in ("legacy", "chunked")
            ...
            for (i, t) in enumerate(xs)
                sp = (mode == "chunked" && par_t1 !== nothing) ?
...
function plot_results(max_threads)
    ...
    for what in (:runtime, :memory), n in N_VALUES
        nkey = "n$n"
        ylabel = what === :runtime ? "forcefreestates_integration  [s]" : "allocated memory  [MiB]"
        ...
        xs, med, _, sd, mem, _ = series(data, nkey, "chunked")
        if !isempty(xs)
            y = what === :runtime ? med : mem
            err = what === :runtime ? sd : zeros(length(y))
            plot!(p, xs, y; yerror=err, marker=:circle, ms=5, lw=2,
                  color=:steelblue, label="chunked Riccati")
            if what === :runtime
                # Ideal strong scaling anchored at the 1-thread chunked-Riccati point.
                plot!(p, xs, y[1] .* xs[1] ./ xs; ls=:dot, lw=1, color=:gray,
                      label="ideal 1/N")
            end
        end

        # Legacy Euler-Lagrange: thread-independent, drawn as a flat band.
        let
            xs0, med0, _, sd0, mem0, _ = series(data, nkey, "legacy")
            if !isempty(xs0)
                ...
                plot!(p, xline, fill(y0, length(xline)); ribbon=fill(e0, length(xline)),
                      ls=:dash, lw=2, color=:tomato, fillalpha=0.15, label="legacy EL (serial)")
            end
        end
        push!(panels, p)
    end
