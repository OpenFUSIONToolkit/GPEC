...
function build_comparison_table(fort, julia, fortran_dir, bench_dir, nn)
    ...
    if !isempty(j_fvec) && !isempty(f_phi_x)
        ...
        for (k, m) in enumerate(m_ctrl)
            ...
            push!(
                lines,
                @sprintf("%-5d  %14.4e  %14s  %-6s  %14.4e  %14s  %-6s",
                    m, f_fx,
    ...
    for (i, (fq_r, fp_r)) in enumerate(zip(f_q_rat, f_psi_rat))
        ...
        jc_julia =
            (row > 0 && row <= length(j_delta) && row <= length(j_i_res) &&
    ...
    for (i, (fq_r, fp_r)) in enumerate(zip(f_q_rat, f_psi_rat))
        ...
        push!(
            lines,
            @sprintf("%-4d  %8.4f  %14s  %14s  %8s  %14s  %14s  %8s",
                q_int, fp_r,
...
function main(argv=ARGS)
    ...
    if use_file_forcing
        ...
        write_gpec_toml_coil(toml_path, fortran_dir, p, abspath(coil_dat_dir), fort["qlim"])
        println("\n[4/5] Wrote gpec.toml (coil forcing mode, qhigh=$(fort["qlim"]) to match Fortran sas_flag)")
    end
