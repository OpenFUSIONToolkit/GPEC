...
mu0 = 4.0e-7 * π
fdw = mu0 .* (fC2 .- fKX)   # Fortran dW density in the 2μ₀δW convention: μ₀(c2_mu0 - k_xin2)
...
@printf "v1 ∫dW dψ (2μ₀δW, real-space)         = %+.6e\n" r1.dW_total
@printf "v1/v2 (both 2μ₀δW → should be ~1)      = %.6f\n" real(r1.dW_total/v2.dW_boundary)
@printf "Fortran 2μ₀δW (∫ μ₀(epf-dst), raw)    = %+.6e\n" fint(fdw)
...
p3 = plot(r1.psi, scale .* r1.dW; lw=2, label="Julia v1 (scaled)", color=:seagreen,
    xlabel="ψ", ylabel="2μ₀ dδW/dψ = μ₀(epf−dst)", title="dW  (2μ₀δW density)", legend=:topleft)
plot!(p3, fP, fdw; lw=2, ls=:dash, label="Fortran recon", color=:black)
fig1 = plot(p1, p2, p3; layout=(3, 1), size=(900, 1000),
    left_margin=12Plots.mm, bottom_margin=4Plots.mm, plot_title="recon 2μ₀δW densities — Julia v1 vs Fortran gpout_recon")
...
fig2 = plot(r1.psi, jcum; lw=2.5, label="Julia v1 ∫dW (scaled)", color=:seagreen,
    xlabel="ψ", ylabel="cumulative 2μ₀δW", title="Cumulative 2μ₀δW(ψ)  (sign = stability)",
...
fig4 = plot(v2.psi, v2.dW_running; lw=2.5, color=:purple, label="v2 running δW(ψ) = Ξ†u₂",
    xlabel="ψ", ylabel="cumulative 2μ₀δW", title="v2 matrix-form 2μ₀δW over full ψ  (endpoint = exact)",
