...
n_core = 4
all_R_c = vcat(Rc[1:n_core]...);
all_Z_c = vcat(Zc[1:n_core]...)
pad = 0.03
xlims!(p_ctr_core, minimum(all_R_c) - pad, maximum(all_R_c) + pad)
ylims!(p_ctr_core, minimum(all_Z_c) - pad, maximum(all_Z_c) + pad)

n_outer = length(psi_contours)
all_R_e = vcat(Rc[(n_outer-2):n_outer]...);
all_Z_e = vcat(Zc[(n_outer-2):n_outer]...)
...
for (sidx, (sname, fn_getter)) in enumerate(rzphi_specs)
    ...
    for (tidx, θ) in enumerate(theta_select)
        ...
        xs = pe_ref.rzphi_xs
        ys = [fn_ref(ψ, θ) for ψ in xs]
        scatter!(p_full, xs, ys; ms=3, markerstrokewidth=0, color=tc, label="")
        let mask = xs .< 0.10
            scatter!(p_core, xs[mask], ys[mask]; ms=3, markerstrokewidth=0, color=tc, label="")
        end
        let mask = xs .> 0.98
            scatter!(p_edge, -log.(1.0 .- xs[mask]), ys[mask]; ms=3, markerstrokewidth=0, color=tc, label="")
        end
    end
