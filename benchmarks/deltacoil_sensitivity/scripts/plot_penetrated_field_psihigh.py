#!/usr/bin/env python3
# plot_penetrated_field_psihigh.py -- matched resonant coupling ||cout|| per rational surface vs the
...
#
# Usage: python3 plot_penetrated_field_psihigh.py results/resonant_coupling_psihigh_<case>.csv
...
csv_path = os.path.abspath(sys.argv[1]) if len(sys.argv) > 1 else \
    os.path.join(here, '..', 'results', 'penetrated_field_psihigh.csv')
...
_case = os.path.splitext(os.path.basename(csv_path))[0].replace('penetrated_field_psihigh', '').strip('_').upper()
...
ax.set_ylabel('penetrated field per surface  (log)', fontsize=11)
ax.set_title('Penetrated field vs psihigh truncation: STRIDE reconnected_flux (solid) vs galerkin bpen (dashed)\n'
             f'{CASE}, n=1  ·  field that penetrates the resistive layer', fontsize=11)
ax.grid(alpha=0.25, which='both')
ax.legend(fontsize=7.6, ncol=2, loc='best')
fig.subplots_adjust(left=0.12, bottom=0.13, right=0.97, top=0.88)

stem = os.path.splitext(os.path.basename(csv_path))[0].replace('penetrated_field', 'pen')
