...
function run_eta(eta, solver)
    ...
    h5open(h5path, "r") do fid
        ...
        # STRIDE resonant match (computed on every gal_match_flag run): per-surface inner Δ and the
        # penetrated resonant field bpen — ONE value per surface (no left/right), extracted from the inner layer.
        rd = haskey(fid, "singular/resonant_match/deltar") ? read(fid, "singular/resonant_match/deltar") : nothing   # (msing,2) STRIDE inner Delta
        rb = haskey(fid, "singular/resonant_match/bpen")   ? read(fid, "singular/resonant_match/bpen")   : nothing   # (msing,ncoil) STRIDE penetrated field
        deltar = Dict{Float64,Float64}(); bpen = Dict{Float64,Float64}()
        ricdeltar = Dict{Float64,Float64}(); ricbpen = Dict{Float64,Float64}()
        rdr = rd === nothing ? nothing : (size(rd, 1) == length(q) ? rd : permutedims(rd))        # rows = msing
        rbr = rb === nothing ? nothing : (size(rb, 1) == length(q) ? rb : permutedims(rb))        # rows = msing
        for s in 1:length(q)
            qk = round(q[s]; digits=2)
            dr === nothing || (deltar[qk] = norm(dr[s, :]))
            rdr === nothing || (ricdeltar[qk] = norm(@view rdr[s, :]))
            rbr === nothing || (ricbpen[qk] = norm(@view rbr[s, :]))
            if bp !== nothing
                b = size(bp, 1) >= size(bp, 2) ? bp : permutedims(bp)
                bpen[qk] = norm(b[:, s])
            end
        end
        (q=sort(round.(q; digits=2)), deltar=deltar, bpen=bpen, ricdeltar=ricdeltar, ricbpen=ricbpen)
    end
end
...
open(csv, "w") do io
    cols = String["eta"]
    # ric_* = STRIDE resonant match (inner Δ and penetrated field bpen, per surface); ray/gal = galerkin-match backends
    for q in allq, m in ("ric_deltar","ric_bpen","ray_deltar","gal_deltar","ray_bpen","gal_bpen"); push!(cols, "$(m)_q$(q)"); end
    println(io, join(cols, ","))
    for (eta, rr, rg) in rows
        ric = rg !== nothing ? rg : rr   # STRIDE match is identical across backends; take whichever ran
        vals = String[string(eta)]
        for q in allq
            push!(vals, ric === nothing ? "" : string(get(ric.ricdeltar, q, "")))
            push!(vals, ric === nothing ? "" : string(get(ric.ricbpen, q, "")))
...
# Per-surface penetrated resonant field ||bpen|| vs eta: STRIDE match (solid) vs Galerkin match (dashed).
# bpen is one value per surface (extracted from the inner layer), directly comparable between the two.
gr()
pbpen = plot(; xscale=:log10, yscale=:log10, xlabel="resistivity  eta  (low eta = high |Q|, to the right)",
    ylabel="penetrated resonant field  ||bpen||", title="Matched penetrated field vs eta ($(basename(base))), n=1",
...
for (i, q) in enumerate(allq)
    col = scols[mod1(i, length(scols))]
    xs = Float64[]; ys = Float64[]; xg = Float64[]; yg = Float64[]
    for (eta, rr, rg) in rows
        ric = rg !== nothing ? rg : rr
        ric !== nothing && haskey(ric.ricbpen, q) && (push!(xs, eta); push!(ys, ric.ricbpen[q]))
        rg  !== nothing && haskey(rg.bpen, q)     && (push!(xg, eta); push!(yg, rg.bpen[q]))
    end
    isempty(xs) || plot!(pbpen, xs, ys; color=col, lw=2, marker=:circle, ms=4, label="q = $q  STRIDE")
    isempty(xg) || plot!(pbpen, xg, yg; color=col, lw=2, ls=:dash, marker=:diamond, ms=4, label="q = $q  Galerkin")
end
xflip!(pbpen)   # low eta (high |Q|) on the right
figdir = joinpath(@__DIR__, "..", "figures"); mkpath(figdir)
figpath = joinpath(figdir, "resistivity_scan_bpen_$tag.png")
savefig(pbpen, figpath)
