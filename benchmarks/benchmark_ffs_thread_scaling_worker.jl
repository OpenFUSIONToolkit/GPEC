...
"""ForceFreeStatesControl for toroidal mode `n`, integration path `mode`
(`:chunked` or `:legacy`), and an `integration_threads` cap of `nth`."""
function make_ctrl(inputs, n, mode, nth)
    ...
    ffs["nn_low"]                = n
    ffs["nn_high"]               = n
    ffs["integration_method"]    = mode === :chunked ? "ChunkedRiccati" : "LegacyEulerLagrange"
    ffs["integration_threads"]   = max(nth, 1)
...
equilibrium is reused (read-only); everything `forcefreestates_integration` mutates
...
"""Benchmark `forcefreestates_integration` for one `(n, mode)` at the worker's thread
...
function bench_path(inputs, n, mode, nth, equil, samples)
    ...
    let
        ...
        FFS.forcefreestates_integration(ctrl, equil, ffit, intr)
    end
    b = @benchmarkable FFS.forcefreestates_integration(c, $equil, f, i) setup = ((c, f, i) = ($builder)()) samples = samples evals = 1 seconds = 3600
    return run(b)
end
...
function run_worker(outfile, samples)
    ...
    # The legacy Euler-Lagrange path is thread-independent — measure it only once (nth=1).
    modes = nth == 1 ? (:legacy, :chunked) : (:chunked,)
