...
Sweeps:
  A. refine ∈ {2, 3, 4, 5, 6, 8}  at β_z = 2.0
  B. β_z   ∈ {0, 0.5, 1, 1.5, 2, 3, 4, 6}  at refine = 5
...
Outputs:
  ...
  pareto.png         — roundtrip error vs runtime (Pareto view)
...
"""Run efit_by_inversion with explicit refine/β_z, return (pe, runtime)."""
...
# Adaptive β_z estimate
ro_ref  = pe_efit.ro
zo_ref  = pe_efit.zo
z_lo_ref = raw_input.zmin
z_hi_ref = raw_input.zmax
topology_ref = Equilibrium.classify_topology(raw_input, raw_input.psio)
_, nz_estimate = let
    nw = length(raw_input.psi_in_xs)
    nh = length(raw_input.psi_in_ys)
    r_span = raw_input.rmax - raw_input.rmin
    z_span = raw_input.zmax - raw_input.zmin
    (max(4, round(Int, 5 * nw)), max(4, round(Int, 5 * nh)))
end
β_z_adaptive = Equilibrium.adaptive_xpt_bz(
    raw_input, ro_ref, zo_ref, z_lo_ref, z_hi_ref, nz_estimate, topology_ref)
@printf("  Adaptive β_z = %.3f (topology = %s)\n", β_z_adaptive, topology_ref)
...
println("\n" * "=" ^ 65)
println("Sweep B: β_z ∈ $bz_vals at refine = $refine_fixed  (adaptive β_z = $(@sprintf("%.3f", β_z_adaptive)))")
println("=" ^ 65)
...
for β_z in bz_vals
    ...
    marker = isapprox(β_z, β_z_adaptive; atol=0.05) ? " ← adaptive" : ""
    @printf("  β_z=%.2f  t=%.2fs  rt=%.2e  q_efit=%.2e  q_hires=%.2e  dv_efit=%.2e  dv_hires=%.2e%s\n",
        β_z, t, rt_err, q_efit, q_hires, dv_efit, dv_hires, marker)
...
try
    ...
    isempty(bz_vline) || vline!(rt2, bz_vline; color=:gray, ls=:dash, label="adaptive β_z")
