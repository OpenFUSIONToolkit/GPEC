...
function run_coil_axis_comparison(;
    ...
    Δx = (field_corrected.x0 - phys_geom.x0) * 1e3
    Δy = (field_corrected.y0 - phys_geom.y0) * 1e3
    Δz = (field_corrected.z0 - phys_geom.z0) * 1e3

    @printf("ΔFIELD_CORR - FIELD_GEOM: dx=%+.4f mm dy=%+.4f mm dz=%+.4f mm\n",
            Δx, Δy, Δz)
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    run_coil_axis_comparison(
        ...
        # ─────────────────────────────────────────────────────────────
        physical_coil_file      = "sparc_pf1u.dat",
        field_coil_file         = "out_3mm.dat",
