...
This note describes the physical and mathematical foundations of `Ballooning.jl`, which
...
`Ballooning.jl` converts this to a first-order matrix equation,
...
``|\nabla\beta|^2 \simeq q'^2\theta^2|\nabla\psi|^2``. `Ballooning.jl` applies the
...
Below, an overbar denotes the period integral used to form the `Ballooning.jl`
``\bar{d}_0`` matrix:
...
new `Ballooning.jl` route constructs the same Mercier index from the averaged
...
finite integration boundary ``\theta_{\max}``. `Ballooning.jl` therefore includes the
...
## Practical implementation in `Ballooning.jl`
...
`Ballooning.jl` therefore adopts the `Baloo.f`-style boundary replacement for the
...
`Ballooning.jl` now computes both the Mercier criterion and the ballooning ``\Delta'``.
...
Mercier calculation and the new `Ballooning.jl` calculation.

![Comparison of Mercier criterion calculations from `Mercier.jl` and `Ballooning.jl`.](assets/ballooning/mercier_comparison.png)
