...
@testset "Sing Tests" begin
    ...
    function read_solutions_3d(fname::String)
        ...
        blocks = Vector{Vector{Vector{Float64}}}(); current = Vector{Vector{Float64}}()
        for s in lines
            t = strip(s); isempty(t) && continue
            if occursin("Solution index", t)
                if !isempty(current)
                    push!(blocks, current); current = Vector{Vector{Float64}}()
                end
                continue
            end
            if occursin("=", t) continue end
            vals = [parse(Float64, replace(x, 'D'=>'E')) for x in split(t)]
            push!(current, vals)
        end
        if !isempty(current) push!(blocks, current) end
        mpert = length(blocks[1]); nsol = length(blocks)
        result = Array{ComplexF64}(undef, mpert, nsol, 2)
        for (j, block) in enumerate(blocks), (i, row) in enumerate(block)
            result[i, j, 1] = complex(row[2], row[3]); result[i, j, 2] = complex(row[4], row[5])
        end
        return result
    end
    ...
    @testset "sing_der" begin
        ...
        ctrl = JPEC.DCON.DconControl(); intr = JPEC.DCON.DconInternal()
        ...
        psifac_dummy = collect(range(0, 1, 10)); points = length(psifac_dummy)
        ...
        fmat .= cholesky(Hermitian(fmat)).L; fmats = copyForSplines(fmat, psifac_dummy)
        ...
        odet.u[:,:,1] .= umat_p1; odet.u[:,:,2] .= umat_p2
