@testset "Dispersion AMR scan + triangulation extraction" begin
    ...
    @testset "find_growth_rates(AMR): extraction does not draw from the global RNG" begin
        # The AMR grid's cocircular points make the Delaunay triangulation non-unique, so the
        # insertion order picks the diagonals and hence the contours; it must not come from the
        # global RNG, which is seeded from entropy at startup.
        using Random
        amr = amr_scan(Q -> ComplexF64(Q) - (0.42 + 0.27im), (-1.0, 1.5), (-0.5, 1.0); nre0=8, nim0=6, passes=2)
        Random.seed!(7)
        find_growth_rates(amr, 1.0)
        after = rand()
        Random.seed!(7)
        @test after == rand()
    end
