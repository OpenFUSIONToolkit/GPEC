@testset "SLAYER LayerInputs (build from equilibrium + profiles)" begin
    ...
    @testset "build_slayer_inputs: returns correct per-surface data" begin
        ...
        # delta_crit_D_R=0.0 bypasses the build_slayer_inputs requirement that sing.restype be
        # pre-populated by ForceFreeStates.resist_eval_all! — the test sings here are
        # minimal stubs without restype, so we supply delta_crit_D_R explicitly.
        ...
        sl = build_slayer_inputs(equil, sings, profiles; bt=2.0, delta_crit_D_R=0.0,
    ...
    @testset "build_slayer_inputs: chi_perp/chi_tor as scalars and callables" begin
        ...
        # Scalar (delta_crit_D_R=0.0 bypasses the sing.restype requirement; see comment above)
        sl_s = build_slayer_inputs(equil, sings, profiles;
                                    bt=2.0, chi_perp=2.0, chi_tor=1.5, delta_crit_D_R=0.0)
        # Callable with matching value
        chi_p(psi) = 2.0 + 0.0*psi
        chi_t(psi) = 1.5 + 0.0*psi
        sl_c = build_slayer_inputs(equil, sings, profiles;
                                    bt=2.0, chi_perp=chi_p, chi_tor=chi_t, delta_crit_D_R=0.0)
        ...
        sl_var = build_slayer_inputs(equil, sings, profiles;
                                      bt=2.0, chi_perp=chi_p_var, chi_tor=1.5, delta_crit_D_R=0.0)
    ...
    @testset "build_slayer_inputs: delta_crit_type propagates and delta_crit_D_R activates offset" begin
        sings = [_mk_sing(psi=0.5, q=2.4, q1=1.2, m=2, n=1)]

        # delta_crit_type=:none and delta_crit_D_R=0.0 → dc_tmp = 0 regardless of delta_crit_D_R
        sl_none = build_slayer_inputs(equil, sings, profiles;
                                       bt=2.0, delta_crit_type=:none, delta_crit_D_R=0.0)
        @test sl_none[1].dc_tmp == 0.0

        # delta_crit_type=:fitzpatrick with delta_crit_D_R = 0 still gives zero
        sl_rf0 = build_slayer_inputs(equil, sings, profiles;
                                      bt=2.0, delta_crit_type=:fitzpatrick, delta_crit_D_R=0.0)
        @test sl_rf0[1].dc_tmp == 0.0

        # delta_crit_type=:fitzpatrick with delta_crit_D_R > 0 → nonzero negative offset
        sl_rf = build_slayer_inputs(equil, sings, profiles;
                                     bt=2.0, delta_crit_type=:fitzpatrick, delta_crit_D_R=0.01)
