...
function load_u_matrix(filename)
    ...
    # For the new 2D format, we need to stack the two components vertically
    # Old format: mat[i, j, k] where k=1,2
    # New format: mat[i or i+N, j] where first N rows are k=1, next N rows are k=2
    mat = zeros(ComplexF64, imax * ncols, jmax)
    for row in data
        i = Int(row[1])
        j = Int(row[2])
        for k in 1:ncols
            ...
            # Stack vertically: k=1 goes to rows 1:imax, k=2 goes to rows imax+1:2*imax
            mat[i + (k-1)*imax, j] = complex(re, im)
        end
    end
    return mat
end
...
@testset "ODE Tests" begin
    @testset "resize_storage!" begin
        ...
        @test size(odet.u_store, 3) == 2 * numsteps_init
        @test size(odet.ud_store, 3) == 2 * numsteps_init
        ...
        @test size(odet.u_store, 3) == 4 * numsteps_init
        @test size(odet.ud_store, 3) == 4 * numsteps_init
    end
    ...
    @testset "transform_u!" begin
        ...
        for i in 1:odet.step
            odet.u_store[:, :, i] .= ComplexF64(i)
            odet.ud_store[:, :, i] .= ComplexF64(i + 0.2)
        end
    ...
    @testset "ode_fixup!" begin
        ...
        odet.u = randn(ComplexF64, 2*mpert, mpert)
        odet.unorm = [norm(odet.u[1:mpert, i]) for i in 1:mpert]
        odet.ifix = ifix
        ...
        # unorm = [norm(odet.u[1:mpert,i]) for i in 1:mpert] in the Fortran to avoid
        ...
        # u[1:2, 1] = [3, 4] (norm = 5)
        # u[1:2, 2] = [1, 0] (norm = 1)
        odet.u[1:mpert, 1] .= [3.0 + 0.0im, 4.0 + 0.0im]
        odet.u[1:mpert, 2] .= [1.0 + 0.0im, 0.0 + 0.0im]
        odet.u[mpert+1:end, :] .= 0.0  # Set second equation to zero for simplicity
        ...
        pivot_idx = argmax(abs.(u_before[1:mpert, 1]))
        @test abs(odet.u[pivot_idx, 1] - u_before[pivot_idx, 1]) < 1e-10
    end
    ...
    @testset "ode_unorm!" begin
        ...
        odet.u = zeros(ComplexF64, 2*mpert, mpert)
        odet.u[1:mpert, 1] .= [3, 4]          # norm = 5
        odet.u[1:mpert, 2] .= [0, 2]          # norm = 2
        ...
        odet.u[1:mpert, 1] .= [1000, 0]   # large norm
        odet.u[1:mpert, 2] .= [1, 0]      # small norm
        JPEC.DCON.ode_unorm!(odet.u, odet, ctrl, intr, false)
        @test odet.new == true  # implies fixup ran

        # Case 5: Trigger fixup via sing_flag
        odet.new = false
        odet.u[1:mpert, 1] .= [1, 0]
        odet.u[1:mpert, 2] .= [1, 0]
    ...
    @testset "OdeState construction" begin
        ...
        @test size(odet.u) == (2*numpert_total, numpert_total)
        @test size(odet.ud) == (2*numpert_total, numpert_total)
        @test size(odet.u_store) == (2*numpert_total, numpert_total, numsteps_init)
        @test size(odet.ud_store) == (2*numpert_total, numpert_total, numsteps_init)
        @test length(odet.psi_store) == numsteps_init
        @test length(odet.q_store) == numsteps_init
        @test size(odet.ca_r) == (2*numpert_total, numpert_total, msing)
        @test size(odet.ca_l) == (2*numpert_total, numpert_total, msing)
