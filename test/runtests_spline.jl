...
@testset "Empty Spline Constructors" begin
    ...
    @test isempty(empty_cs_f64._interps)  # Empty interps indicates uninitialized
    ...
    @test isempty(empty_cs_c64._interps)
    ...
    @test isempty(empty_cs_default._interps)

    # Test empty BicubicSpline
    empty_bs = JPEC.Spl.empty_BicubicSpline()
    @test isempty(empty_bs._interps)
    @test empty_bs.mx == 0
    @test empty_bs.my == 0
...
@testset "Spline Replacement" begin
    ...
    @test isempty(empty_cs._interps)
    ...
    @test !isempty(real_spline._interps)
