...
@testset "ODE Tests" begin
    ...
    @testset "compute_tols" begin
        ...
        intr.msing = 2
        intr.sing = [GeneralizedPerturbedEquilibrium.ForceFreeStates.SingType(), GeneralizedPerturbedEquilibrium.ForceFreeStates.SingType()]
        intr.sing[1].q = 2.0
        intr.sing[1].n = [1]
        intr.sing[2].q = 3.0
        intr.sing[2].n = [1]

        odet = GeneralizedPerturbedEquilibrium.ForceFreeStates.OdeState(mpert, 10, 10, 2)

        # Test 1: Far from singular surface (singfac > crossover)
        ising = 1
        odet.q = 1.5  # Far from q=2.0
        rtol = GeneralizedPerturbedEquilibrium.ForceFreeStates.compute_tols(ctrl, intr, odet, ising)
        ...
        odet = GeneralizedPerturbedEquilibrium.ForceFreeStates.OdeState(mpert, 10, 10, 0)
        ising = 1
        odet.q = 2.0
